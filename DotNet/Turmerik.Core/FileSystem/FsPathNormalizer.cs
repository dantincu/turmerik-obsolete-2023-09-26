...
namespace Turmerik.Core.FileSystem
{
    ...
    public class FsPathNormalizer : IFsPathNormalizer
    {
        ...
        public IFsPathNormalizerResult TryNormalizePath(IFsPathNormalizerOpts opts)
        {
            ...
            var immtbl = this.TryNormalizePath(d);
            ...
            int maxStartingSlashesAllowed = 2)
        {
            ...
            var immtbl = this.TryNormalizePath(opts);
            return immtbl;
        }
        ...
        private IFsPathNormalizerResult TryNormalizePath(TempData d)
        {
            ...
            try
            {
                if (this.Validate(mtbl, pathIsNotNull, NULL_INPUT_PATH_ERR_MSG))
                {
                    this.ValidateAgainstSurroundingWhiteSpaces(mtbl, ref path);
                }
                    ...
                    if (idx > 0)
                    {
                        ...
                        if (rest.First() == '/')
                        {
                            ...
                            if (firstPartTrimmed.LastOrDefault() == ':' && rest.StartsWith("//"))
                            {
                                ...
                                this.ValidateAgainstSurroundingWhiteSpaces(mtbl, firstPart, firstPartTrimmed);
                                this.ValidateAgainstInvalidFileNameChars(mtbl, firstPartTrimmed);
            ...
            TempData d)
        {
            ...
            for (int i = 0; i < pathPartsLen; i++)
            {
                ...
                this.ValidateAgainstSurroundingWhiteSpaces(mtbl, ref segment);
                ...
                else
                {
                    ...
                    if (cIdx >= 0)
                    {
                        if (this.Validate(mtbl,
                            this.IsWinDriveLetterSegment(
                            o.IsUnixStyle, i == 0,
                            cIdx, segment, segLen),
                    ...
                    {
                        this.ValidateAgainstInvalidWhiteSpaces(mtbl, segment);
                        this.ValidateAgainstDotsNearWhiteSpaces(mtbl, segment);

                        int dotsCount;

                        if (this.ContainsOnlyDots(segment, out dotsCount))
                        {
                            if (dotsCount == 2)
                            {
                                if (this.Validate(mtbl,
                        ...
                        {
                            this.Validate(mtbl, segment.Last() != '.', SEGMENT_ENDING_IN_DOT_ERR_MSG);
        ...
        {
            bool retVal = this.Validate(mtbl,
        ...
        private bool ValidateAgainstSurroundingWhiteSpaces(FsPathNormalizerResultMtbl mtbl, ref string segment)
        {
            ...
            bool retVal = this.ValidateAgainstSurroundingWhiteSpaces(mtbl, segment, segmentTrimmed);
        ...
        private class TempData
        {
            ...
            public TempData(FsPathNormalizerOptsImmtbl opts)
            {
                ...
                this.Mtbl = new FsPathNormalizerResultMtbl();
                this.SegmentsList = new List<string>();
            }
